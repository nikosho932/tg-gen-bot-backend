...
bot.on("message", async (msg) => {
  ...
  if (text === "👥 Пригласить друзей") {
    const invite = "Пригласи друзей и получи бонус! (Функция в разработке)";
  ...
  if (text === "🎨 Генерировать изображения") {
    try {
      ...
      if (!user.photos_added) {
        return bot.sendMessage(chatId, "📸 Сначала добавьте исходники в личном кабинете (кнопка «Загрузить исходники»).");
      }
      ...
      if (user.tokens < cost) {
        return bot.sendMessage(chatId, `❌ У вас недостаточно токенов. Стоимость генерации — ${cost} токенов.`);
      }
...
bot.on("callback_query", async (query) => {
  ...
  if (data === "support") {
    await bot.answerCallbackQuery(query.id, { text: "Связаться с поддержкой: support@example.com" });
    return;
  }
});
